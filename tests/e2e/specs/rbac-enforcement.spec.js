...
test.describe.serial('permission enforcement', () => {
  ...
  test('admin role deletion is refused (protected slug)', async () => {
    ...
    // No delete offered for protected role — or deletion refused server-side
    const del = adminRow.locator('input[value="Delete"]');
    if (await del.count()) {
      page.on('dialog', (d) => d.accept());
      await del.click();
      await gotoRbacPage(page, 'roles');
      await expect(page.locator('tr', { hasText: /^admin/ })).toHaveCount(1);
    }
  });
});
