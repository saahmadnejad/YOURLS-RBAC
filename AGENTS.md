...
- PHP 8.1+ locally, PHP 8.4 in Docker (uses `str_starts_with`, named returns `int|bool`, `readonly` props).
...
| `docker compose up --build` | Start dev environment (YOURLS + MariaDB) |
...
- `docker compose up --build` — builds and starts YOURLS + MariaDB.
- Plugin source is mounted as read-only volumes for live code iteration.
- Config (`user/config.php`) is auto-generated by `docker/entrypoint.sh`
